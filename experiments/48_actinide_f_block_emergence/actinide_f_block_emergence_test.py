...
OUT = Path("results/research_final/actinide_f_block_emergence_test")
...
ACTINIDES = [
    ...
    (92, "U",  3, 1, 2, "early_f"),
    (93, "Np", 4, 1, 2, "early_f"),
    (94, "Pu", 6, 0, 2, "half_f_approach"),
    (95, "Am", 7, 0, 2, "half_f"),
    (96, "Cm", 7, 1, 2, "half_f"),
    (97, "Bk", 9, 0, 2, "late_f"),
    (98, "Cf", 10, 0, 2, "late_f"),
    (99, "Es", 11, 0, 2, "late_f"),
    (100, "Fm", 12, 0, 2, "late_f"),
    (101, "Md", 13, 0, 2, "filled_f_approach"),
    (102, "No", 14, 0, 2, "filled_f"),
    (103, "Lr", 14, 1, 2, "post_f"),
]
...
def make_sample(Z, symbol, f_count, d_count, s_count, family, perturbation, replicate):
    ...
    oxidation_flexibility = (
        0.45 * np.exp(-abs(f_count - 3) / 4.0)
        + 0.25 * d_fill
        + 0.20 * (1.0 - filled_f_strength)
        + 0.10 * (1.0 - f_fill)
        + perturbation * rng.normal(0, 0.01)
    )
    ...
    recurrence_signature = (
        0.22 * f_fill
        + 0.18 * f_symmetry
        + 0.18 * shielding_strength
        + 0.14 * magnetic_moment_proxy
        + 0.12 * oxidation_flexibility
        + 0.08 * relativistic_contraction
        + 0.08 * s_fill
        + perturbation * rng.normal(0, 0.01)
    )

    actinide_stability = (
        0.28 * f_block_activation
        + 0.22 * recurrence_signature
        + 0.18 * shielding_strength
        + 0.14 * f_symmetry
        + 0.10 * actinide_contraction
        + 0.08 * relativistic_contraction
    )
    ...
    return {
        ...
        "symbol": symbol,
        "period": 7,
        "block": "f" if symbol not in ["Ac", "Th", "Lr"] else "d/f_boundary",
        "family": family,
        ...
        "actinide_contraction": actinide_contraction,
        "relativistic_contraction": relativistic_contraction,
        "shielding_strength": shielding_strength,
        "magnetic_moment_proxy": magnetic_moment_proxy,
        "oxidation_flexibility": oxidation_flexibility,
        "f_block_activation": f_block_activation,
        "recurrence_signature": recurrence_signature,
        "actinide_stability": actinide_stability,
...
mean_contraction_slope = float(
    np.polyfit(
        df.groupby("Z")["Z"].mean(),
        df.groupby("Z")["actinide_contraction"].mean(),
        1
    )[0]
)

relativistic_slope = float(
    np.polyfit(
        df.groupby("Z")["Z"].mean(),
        df.groupby("Z")["relativistic_contraction"].mean(),
        1
    )[0]
)
...
by_element = df.groupby(["Z", "symbol"]).agg(
    ...
    mean_actinide_contraction=("actinide_contraction", "mean"),
    mean_relativistic_contraction=("relativistic_contraction", "mean"),
    mean_shielding_strength=("shielding_strength", "mean"),
    mean_magnetic_moment_proxy=("magnetic_moment_proxy", "mean"),
    mean_oxidation_flexibility=("oxidation_flexibility", "mean"),
    mean_f_block_activation=("f_block_activation", "mean"),
    mean_recurrence_signature=("recurrence_signature", "mean"),
    mean_actinide_stability=("actinide_stability", "mean"),
).reset_index().sort_values("Z")

by_family = df.groupby("family").agg(
    count=("family", "count"),
    elements=("symbol", lambda x: ",".join(sorted(set(x), key=lambda s: [e[1] for e in ACTINIDES].index(s)))),
    mean_f_count=("f_count", "mean"),
    mean_f_symmetry=("f_symmetry", "mean"),
    mean_actinide_contraction=("actinide_contraction", "mean"),
    mean_relativistic_contraction=("relativistic_contraction", "mean"),
...
if (
    ...
    verdict = "actinide_f_block_emergence_supported"
elif (
    family_accuracy >= 0.85
    and f_block_detection >= 0.75
    and mean_contraction_slope < 0
):
    verdict = "weak_actinide_f_block_emergence"
else:
    verdict = "actinide_f_block_emergence_not_supported"

summary = pd.DataFrame([{
    "num_actinides": len(ACTINIDES),
    "num_samples": len(df),
    ...
    "mean_actinide_stability": float(df["actinide_stability"].mean()),
    "verdict": verdict,
}])

df.to_csv(OUT / "actinide_f_block_samples.csv", index=False)
summary.to_csv(OUT / "actinide_f_block_summary.csv", index=False)
by_element.to_csv(OUT / "actinide_f_block_by_element.csv", index=False)
by_family.to_csv(OUT / "actinide_f_block_by_family.csv", index=False)
...
plt.title("BST actinide f-block emergence space")
plt.xlabel("PC1")
plt.ylabel("PC2")
plt.legend(fontsize=7)
plt.tight_layout()
plt.savefig(OUT / "actinide_f_block_projection.png", dpi=220)
plt.close()

plt.figure(figsize=(12, 5))
plt.plot(by_element["symbol"], by_element["mean_f_block_activation"], marker="o", label="f-block activation")
plt.plot(by_element["symbol"], by_element["mean_actinide_contraction"], marker="x", label="actinide contraction")
plt.plot(by_element["symbol"], by_element["mean_relativistic_contraction"], marker="s", label="relativistic contraction")
plt.ylabel("BST score")
plt.title("BST actinide f-block activation and relativistic contraction")
plt.legend()
plt.tight_layout()
plt.savefig(OUT / "actinide_f_block_scores.png", dpi=220)
plt.close()

print("\n=== BST ACTINIDE f-BLOCK EMERGENCE TEST ===\n")
print(summary.to_string(index=False))

print("\nActinide f-block by family:")
print(by_family.to_string(index=False))

print("\nActinide f-block by element:")
print(by_element.to_string(index=False))

print(f"\n[OK] wrote {OUT / 'actinide_f_block_samples.csv'}")
print(f"[OK] wrote {OUT / 'actinide_f_block_summary.csv'}")
print(f"[OK] wrote {OUT / 'actinide_f_block_by_element.csv'}")
print(f"[OK] wrote {OUT / 'actinide_f_block_by_family.csv'}")
print(f"[OK] wrote {OUT / 'actinide_f_block_projection.png'}")
print(f"[OK] wrote {OUT / 'actinide_f_block_scores.png'}")
print("[DONE] actinide f-block emergence test complete")